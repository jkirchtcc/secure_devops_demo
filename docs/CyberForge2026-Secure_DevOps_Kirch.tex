...
\begin{itemize}
  \item We are going to need our SSH Keys for this
...
\begin{itemize}
  \item Update 2 files:
  \begin{itemize}
    \item \textbf{\texttt{\textasciitilde/.ssh/config}}
    \item \textbf{\texttt{inventory.ini}}
  \end{itemize}
\end{itemize}
\vspace{0.3cm}
\begin{termsmall}
...
\begin{termsmall}
ansible_user@ansible:~/secure_devops_demo$ ansible-playbook sshd_hardening-servers.yml

PLAY [Configure SSHD for Ubuntu 22.04, 24.04, and 24.10] *****************
...
fatal: [192.168.122.101]: UNREACHABLE! => {"changed": false, "msg": "Failed
  ...
  denied (publickey).", "unreachable": true}
fatal: [192.168.122.102]: UNREACHABLE! => {"changed": false, "msg": "Failed
  to connect to the host via ssh: ansible_user@192.168.122.102: Permission
  denied (publickey).", "unreachable": true}
fatal: [192.168.122.103]: UNREACHABLE! => {"changed": false, "msg": "Failed
  to connect to the host via ssh: ansible_user@192.168.122.103: Permission
  denied (publickey).", "unreachable": true}

PLAY RECAP ****************************************************************
192.168.122.101    : ok=0  changed=0  unreachable=1  failed=0
192.168.122.102    : ok=0  changed=0  unreachable=1  failed=0
192.168.122.103    : ok=0  changed=0  unreachable=1  failed=0
\end{termsmall}
\end{frame}
...
% ==============================================================
\begin{frame}[fragile]{Ahhh, I need an ssh client config}
\begin{termsmall}
ansible_user@ansible:~/secure_devops_demo$ ls ~/.ssh
DemoSSHKey  DemoSSHKey.pub  authorized_keys  known_hosts  config
\end{termsmall}
\vspace{0.2cm}
\begin{termsmall}
