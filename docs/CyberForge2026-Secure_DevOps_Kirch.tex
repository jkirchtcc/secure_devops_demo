...
\tcbuselibrary{listings,skins,minted}
...
% ── Minted (Pygments) syntax-highlighted code boxes ──────────────────────────
% before upper={\color{termwhite}} ensures bare text tokens (no Pygments color)
% inherit white instead of beamer's black normal text color. In listing only
% mode, the listing content lives in the upper area of the tcolorbox.
\tcbset{mintedbase/.style={
  colback=darkterm, colframe=darkterm,
  arc=4pt, boxrule=0pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  listing only,
  before upper={\color{termwhite}},
}}
% YAML — for Ansible playbooks
\newtcblisting{codeyaml}{mintedbase,
  minted language=yaml,
  minted options={style=monokai,fontsize=\scriptsize,breaklines=true},
}
\newtcblisting{codeyamlsmall}{
  colback=darkterm, colframe=darkterm,
  arc=4pt, boxrule=0pt,
  ...
  before upper={\color{termwhite}},
  minted language=yaml,
  minted options={style=monokai,fontsize=\tiny,breaklines=true,baselinestretch=0.85},
}
% Bash — for shell commands and scripts
\newtcblisting{codebash}{mintedbase,
  minted language=bash,
  minted options={style=monokai,fontsize=\small,breaklines=true},
}
\newtcblisting{codebashsmall}{
  colback=darkterm, colframe=darkterm,
  arc=4pt, boxrule=0pt,
  left=4pt, right=4pt, top=2pt, bottom=2pt,
  listing only,
...
% INI — for ansible.cfg and inventory files
\newtcblisting{codeini}{mintedbase,
  minted language=ini,
  minted options={style=monokai,fontsize=\scriptsize,breaklines=true},
}
\newtcblisting{codeinismall}{
  colback=darkterm, colframe=darkterm,
  arc=4pt, boxrule=0pt,
  left=4pt, right=4pt, top=2pt, bottom=2pt,
  listing only,
...
% SSH config — lstlistings (no Pygments lexer available; lstlistings is simpler and reliable)
\newtcblisting{codessh}{
  colback=darkterm, colframe=darkterm,
  arc=4pt, boxrule=0pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  listing only, listing engine=listings,
...
% ── Emphasis colors for Ansible output in terminal blocks ────────────────────
\lstdefinestyle{terminal}{
  basicstyle=\ttfamily\scriptsize\color{termwhite},
  emph={ok,changed,CHANGED},
  emphstyle=\color{termgreen}\bfseries,
  emph={[2]fatal,FAILED,UNREACHABLE},
  emphstyle={[2]\color{termred}\bfseries},
}
\lstdefinestyle{terminalsmall}{
  basicstyle=\ttfamily\tiny\color{termwhite},
  emph={ok,changed,CHANGED},
  emphstyle=\color{termgreen}\bfseries,
  emph={[2]fatal,FAILED,UNREACHABLE},
  emphstyle={[2]\color{termred}\bfseries},
...
\vspace{0.1cm}
\begin{codebash}
ssh-keygen -t ed25519 -C DemoSSHKey \
  -f /home/ansible_user/.ssh/DemoSSHKey
\end{codebash}
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{\texttt{-f \ldots/DemoSSHKey}}: Saves keys to \texttt{\textasciitilde/.ssh/DemoSSHKey} and \texttt{.pub}.
  \end{itemize}
\end{itemize}
\end{frame}
...
% ==============================================================
\begin{frame}{Logout \& Login}
\begin{center}
  \includegraphics[width=\textwidth]{screenshots/Logout_Login-A.png}
\end{center}
\end{frame}
...
\vspace{0.1cm}
\begin{termsmall}
ansible_user@ansible:~/secure_devops_demo$ ansible-playbook -i inventory.ini ping.yml
...
\begin{itemize}
  ...
  \item It failed --- we need a password to use sudo
  \item We need a secure way to store and provide that password
\end{itemize}
\end{column}
\end{columns}
\vspace{0.2cm}
...
[WARNING]: Error getting vault password file (default):
  The vault password file /nonexistent was not found
[ERROR]: The vault password file /nonexistent was not found
\end{termsmall}
\end{frame}
...
vault_password_file =
    /home/ansible_user/bin/get_vault_pass.sh
\end{codeini}
\end{column}
...
\begin{itemize}
  \item 3 local VMs on QEMU/KVM (no internet needed) --- cloud-init injects your public key automatically
\end{itemize}
\vspace{0.15cm}
\begin{columns}[T]
\begin{column}{0.44\textwidth}
\textbf{BEFORE:} \texttt{inventory.ini}
\vspace{0.1cm}
\begin{codeinismall}
[servers]
\end{codeinismall}
...
\end{column}
\begin{column}{0.52\textwidth}
\textbf{AFTER:} \texttt{inventory.ini}
\vspace{0.1cm}
\begin{codeinismall}
[servers]
target-1 ansible_host=192.168.122.101
target-2 ansible_host=192.168.122.102
target-3 ansible_host=192.168.122.103
\end{codeinismall}
\end{column}
\end{columns}
\end{frame}
...
{\tiny\texttt{ansible servers -m setup}}\\
{\tiny\texttt{\ \ -a "filter=ansible\_distribution*"}}
\vspace{0.1cm}
\begin{termsmall}
...
\end{column}
\begin{column}{0.32\textwidth}
{\tiny\texttt{ansible servers -m shell}}\\
{\tiny\texttt{\ \ -a "df -h"}}
\vspace{0.1cm}
\begin{termsmall}
192.168.122.101 | CHANGED
Filesystem  Size Avail
/dev/vda1   9.6G  7.9G
tmpfs        46M   45M
...
192.168.122.102 | CHANGED
...
192.168.122.103 | CHANGED
...
\end{termsmall}
\end{column}
\begin{column}{0.32\textwidth}
{\tiny\texttt{ansible servers}}\\
{\tiny\texttt{\ \ -a "ss -tuln"}}
\vspace{0.1cm}
\begin{termsmall}
192.168.122.101 | CHANGED
